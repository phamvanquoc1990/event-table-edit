...
class EventtableeditModelEtetable extends JModelList
{
	...
	protected function populateState()
	{
		...
		$app = JFactory::getApplication('site');
		$pk = $this->params->get('tablenumber');
	...
	public function getHeads() {
		if ($this->heads === null) {
			try {
				...
				$query->where('a.table_id = ' . $this->state->get('etetable.id'));
	 ...
	 */
	 ...
	 protected function getRowsQuery() {
	 	...
	 	$orderDirn	= $this->state->get('list.direction');	
 		 $tid = $this->state->get('etetable.id');
	 ...
	 private function filterRows() {
		$this->filter = JRequest::getVar('filterstring', '');
	...
	private function parseCell($cell, $colCount) {
		...
		/*if ($this->filter != '' && !JRequest::getBool('print')) {
			$cell = eregi_replace($this->filter, '<span class="filter-highlight">' . $this->filter . '</span>', $cell);
		}*/
